...
{
    public class VT340LocationMessage : GTSLocationMessage
    {
        internal VT340LocationMessage(string tid, byte[] message, int bytesRead)
        ...
        protected override void objectify(byte[] message, int bytesRead)
        {
            ...
            Utilities.writeLine("Debug 20: Objectification Report for VT340");
            Utilities.writeLine("Debug 21: Raw Text: " + rawTextData);

            try
            {
                Utilities.writeLine("Validity: " + _data[7]);
                if (_data[7] == "V") throw new Exception("Reading is not valid, halting saving process");
                isValid = true;
                // TODO need to update so that if not valid the thread will stop processing and wait for next message

                // need to update code to pull and process date time
                clientDateTime = 
                    DateTime.ParseExact(_data[6], "yyMMddHHmmss", 
                    System.Globalization.CultureInfo.InvariantCulture);

                Utilities.writeLine("Client DateTime: " + clientDateTime);

                double.TryParse(_data[11], out direction);
                Utilities.writeLine("Direction: " + direction);

                // need to put in better exception handling for this section
                speed = double.Parse(_data[10]); // speed in km
                Utilities.writeLine("Speed: " + speed);
                ...
                latitude.Decimal = double.Parse(_data[4]);
                longitude.Decimal = double.Parse(_data[5]);
                ...
                Utilities.writeLine("Debug 22: Message Prcessed!" );

            }
            catch (Exception e)
            {
                //Need to log exceptions properly
                Utilities.writeLine("Error 5: "+e.Message + ">>>>\n" + e.StackTrace);
            }
