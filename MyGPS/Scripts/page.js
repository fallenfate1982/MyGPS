$(document).ready(function () {
    ...
    $(document).on('click', '#table_paths tbody tr button', function () {
        ...
        if (action === 'delete') {
            ...
            if (yn) {
                ...
                $("#table_paths").hide();
                $("#hdId").val("0");
                $('#polygon_main_add').prop('disabled', false);*/
            }
        }
    ...
    $('#polygon_reset').click(function () {
        var yn = confirm('All' +
            ' paths will be removed, do you want to continue ?');
        if (yn) {
            $("#table_paths tbody").html('');
            $('#table_paths').hide();
    ...
    function pathAppend2Table(path_k) {
        $('#table_paths tbody').append('<tr><td><input type="text" class="input" value="' + $("#area_of_interest").val() + '" /></td><td><button class="btn btn-mini btn-info" data-action="zoom">Zoom to</button> <button class="btn btn-mini btn-danger" data-action="delete">Delete</button></td></tr>');
    }
